...
import { PortConflictError, ValidationError } from '../utils/errors';
...
describe('InstanceManager', () => {
  ...
  let mockRepo: jest.Mocked<InstancesRepository>;
  let mockSystemd: jest.Mocked<SystemdService>;

  beforeEach(() => {
    mockRepo = new InstancesRepository() as jest.Mocked<InstancesRepository>;
    mockSystemd = new SystemdService(mockRepo) as jest.Mocked<SystemdService>;
    manager = new InstanceManager(mockRepo, mockSystemd);
  });

  const dummyInstances: ServerInstance[] = [
    {
  ...
  it('checkPortConflicts should return conflicts if port is used', async () => {
    ...
    mockSystemd.execute.mockResolvedValue({ stdout: 'active', stderr: '' } as any);
