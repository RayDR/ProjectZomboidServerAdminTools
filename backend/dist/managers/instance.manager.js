...
class InstanceManager {
    ...
    async getRunningPortConflicts(gamePort, rconPort, excludeInstanceId) {
        const instances = await this.listInstances();
        const conflicts = [];
        for (const inst of instances) {
            if (inst.id === excludeInstanceId)
    ...
    async createInstanceFromVersion(branchId, name, gamePort, rconPort, force = false, allowUnknownBranch = false, taskId) {
        ...
        if (await this.repository.exists(id)) {
            throw new errors_1.ValidationError(`La instancia con ID ${id} ya existe / Instance with ID ${id} already exists`);
        }
        ...
        if (conflictWarnings.length > 0) {
            ...
            if (taskId) {
                task_manager_1.taskManager.addLog(taskId, `[warn] ${conflictWarnings.join(' | ')}`);
            }
        }
        if (taskId)
        ...
        if (!steamCmdInfo.exists || !steamCmdInfo.executable) {
            if (taskId)
                task_manager_1.taskManager.updateTaskStatus(taskId, 'failed', 0, 'SteamCMD no está instalado o no es ejecutable en la ruta configurada / SteamCMD is not installed or executable at the configured path.');
            throw new errors_1.AppError('SteamCMD no está instalado o no es ejecutable en la ruta configurada / SteamCMD is not installed or executable at the configured path.', 'STEAMCMD_NOT_FOUND', 500);
        }
        try {
            if (taskId)
                ...
                child.on('close', (code) => {
                    if (code === 0) {
                        ...
                        if (combinedOutput.includes('permission denied') || combinedOutput.includes('a password is required') || combinedOutput.includes('sudo:')) {
                            reject(new errors_1.AppError('Permiso denegado al crear la instancia. Verifica sudoers y permisos del sistema de archivos / Permission denied while creating instance. Verify sudoers and filesystem permissions.', 'PERMISSION_DENIED', 403));
                        }
                        else {
                            reject(new errors_1.AppError(`La instalación con SteamCMD falló para la rama '${branchId}' (código ${code}). ${details}`, 'INSTALL_FAILED', 500));
                        }
                    }
                });
                child.on('error', (err) => {
                    reject(new errors_1.AppError(`No se pudo iniciar el script de instalación / Failed to start setup script: ${err.message}`, 'INSTALL_FAILED', 500));
                });
            });
        }
        catch (error) {
    ...
    async addInstance(name, pathDir, serviceName, gamePort = 0, rconPort = 0, force = false, customPzName, customIniPath, customSavePath, customDbPath) {
        try {
            ...
            throw new errors_1.ValidationError(`El directorio ${pathDir} no es accesible / Directory ${pathDir} is not accessible`);
        }
        const id = (0, instanceName_1.sanitizeInstanceName)(name).toLowerCase();
        if (!id)
            throw new errors_1.ValidationError('Nombre de instancia inválido / Invalid instance name');
        ...
        if (!force) {
            ...
            if (conflicts.length > 0) {
                throw new errors_1.PortConflictError(conflicts.map((conflict) => conflict.replace(/^/, '')));
            }
        }
    ...
    async retryInstanceInstallation(instanceId, taskId) {
        ...
        if (listedInstance.running) {
            throw new errors_1.AppError('Retry installation is not allowed while the instance is running.', 'RETRY_NOT_ALLOWED', 409);
        }
        ...
        if (!hasFailedInstall && !isBroken) {
            throw new errors_1.AppError('Retry installation is only allowed for failed or broken instances.', 'RETRY_NOT_ALLOWED', 409);
        }
        const instance = await this.getInstance(instanceId);
        const branchId = instance.version || 'public';
        const resolvedBranch = await (0, steamcmd_service_1.resolveBranchById)(branchId, { allowUnknown: true });
        const steamCmdInfo = await (0, steamcmd_service_1.resolveSteamCmdPath)();
        ...
        try {
            if (taskId)
                task_manager_1.taskManager.addLog(taskId, `[instances.retry] Re-running installation for ${instance.name}...`);
            if (taskId)
                task_manager_1.taskManager.updateTaskStatus(taskId, 'running', 10);
            await new Promise((resolve, reject) => {
                const child = (0, child_process_1.spawn)('sudo', [
                ...
                child.on('close', (code) => {
                    if (code === 0) {
                        ...
                        const details = summarizeSetupOutput(errorBuffer, outputBuffer);
                        reject(new errors_1.AppError(`La reinstalación falló para '${branchId}' (código ${code}). ${details}`, 'INSTALL_FAILED', 500));
                    }
                });
                child.on('error', (err) => {
                    reject(new errors_1.AppError(`No se pudo iniciar el script de reinstalación: ${err.message}`, 'INSTALL_FAILED', 500));
                });
            });
            await this.updateInstance(instanceId, {
                installationTaskId: taskId,
        ...
        if (taskId) {
            task_manager_1.taskManager.setTaskResult(taskId, refreshed);
        }
        return refreshed;
    }
    ...
    async performSystemdAction(instanceId, action) {
        ...
        if (action === 'start') {
            ...
            if (portConflicts.length > 0) {
                throw new errors_1.PortConflictError(portConflicts);
            }
    ...
    async deleteInstance(instanceId, createBackup, force = false) {
        ...
        if (instance.isLocked) {
            throw new errors_1.AppError('Instance is locked and cannot be deleted until unlocked.', 'INSTANCE_LOCKED', 423);
        }
