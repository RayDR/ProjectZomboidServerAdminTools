...
CONFIG_FILE="${1:-"$SCRIPT_DIR/config.template.json"}"
BACKEND_LINK="$SCRIPT_DIR/../backend"
...
PZ_BE_PORT=$(jq -r .backend_port "$CONFIG_FILE")
PORT=$(jq -r .frontend_port "$CONFIG_FILE")
...
NEXT_PUBLIC_FE_PORT=$PZ_FE_PORT
