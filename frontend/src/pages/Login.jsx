  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data?.user) {
        ...
        localStorage.setItem('mustChangePassword', response.data.user.mustChangePassword ? '1' : '0');
      }
      window.dispatchEvent(new Event('pzwebadmin-auth-changed'));
      navigate(response.data?.user?.mustChangePassword ? '/change-password' : '/');
    } catch (err) {
      ...
      >
        ...
        <CRTScreen className="p-8">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-4xl font-bold text-text font-title mb-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-text mb-2 font-bold uppercase text-sm flex items-center space-x-2">
